...
export default function LabelBar ({children, themeColor=Color.fire_red, style, ...restOfProps}: PanelBodyProps) {
  ...
  const panelBodyStyle:React.CSSProperties = {
    ...
    linear-gradient(rgb(${themeColor[0]}, ${themeColor[1]}, ${themeColor[2]}), rgb(${Math.max(themeColor[0] + GRADIENT_RANGE, 0)}, ${Math.max(themeColor[1] + GRADIENT_RANGE, 0)}, ${Math.max(themeColor[2] + GRADIENT_RANGE, 0)})),
    radial-gradient(400% 600% at 100% 100%, rgba(0, 0, 0, 0) 22%, rgba(255, 255, 255, ${GLARE_TRANSPARENCY}) 22.5%),
    linear-gradient(to bottom, rgba(${themeColor[0]}, ${themeColor[1]}, ${themeColor[2]}, ${GLARE_TOP_FADE}), rgba(0,0,0,0))
    `,
    backgroundBlendMode: "overlay",
    borderRadius: "4px",
    border:`1px solid rgb(${Math.max(themeColor[0] + GRADIENT_RANGE, 0)}, ${Math.max(themeColor[1] + GRADIENT_RANGE, 0)}, ${Math.max(themeColor[2] + GRADIENT_RANGE, 0)})`,
    padding:"3px",
    color:"white",
